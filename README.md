...
- позволяет вручную задать курс рубля для любой страны
...
<p>
  <img src="docs/screenshots/settings-1.png" alt="Настройки: страны и ручной курс" width="48%">
  <img src="docs/screenshots/settings-2.png" alt="Настройки: расчет и кеш" width="48%">
</p>
...
Если в настройках указан ручной курс для страны, расширение считает рублевую цену напрямую по формуле `цена в валюте магазина * курс`. Пустое поле возвращает автоматический расчет по картам.

![Пример расчета, какой набор карт лучше купить](docs/screenshots/cart.png)
...
- Расширение не совершает покупки и не помогает обходить региональные, платежные или аккаунтные ограничения PS Store.
- Цены и скидки берутся из публичных страниц PS Store и могут стать недоступны или отображаться неточно, если Sony изменит разметку сайта.
- Рублевая цена является расчетной: она зависит от доступных предложений карт пополнения и не является финансовой рекомендацией.
- Ручной курс применяется только локально в браузере и отвечает за расчет выбранной страны до очистки поля или сброса настроек.
- Запросы к PS Store и Игромиру выполняются напрямую из браузера пользователя.
...
MIT. История изменений: [CHANGELOG.md](CHANGELOG.md).
